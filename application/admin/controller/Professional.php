...
use \think\Session;
use think\Request;
use app\admin\model\Professional as ProfessionalModel;
use app\admin\model\Picture;
use app\admin\model\ProfessionalPic;
...
class Professional extends Base
{
    ...
    public function index()
    {
    	...
    	$list = ProfessionalModel::where($where)->order('status DESC,id DESC')->paginate(15,false,array('query'=>$data));
    ...
    function add(){
    	...
    	if(Request::instance()->post()){
	    	...
	    	if($professional->save($data)){
	    		if(!empty($pic)){
	    			foreach ($pic as $k => $v) {
	    				ProfessionalPic::insert(['professional_id'=>$professional['id'],'url'=>$v,'time'=>time(),'sort'=>$k]);
	    			}
	    		}
