...
class Service extends Base
{
    ...
    public function index()
    {
    	...
    	$list = ServiceTemplateModel::where($where)->order('status DESC,id DESC')->paginate(15,false,array('query'=>$data));
    ...
    function add(){
    	...
    	if(Request::instance()->post()){
	    	...
	    	$service_template = new ServiceTemplateModel;
	    	if($service_template->save($data)){
    ...
    function edit(){
    	...
    	if(Request::instance()->post()){
	    	$data = input('post.');
	    	$service_template = ServiceTemplateModel::get($data['id']);
	    	if($service_template->save($data)){
		...
		//
		$service_template = ServiceTemplateModel::get($id);
		$this->assign('service_template',$service_template);
    ...
    function del(){
		...
		$id = $data['id'];
    	if($id&&is_numeric($id)){
    		$user = ServiceTemplateModel::get($id);
